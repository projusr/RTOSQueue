/*
    ...
    Free RTOS  
	Illuminating LED with  Push Button 
	Used synchronisation resource  among tasks  to avoid multiple presses
*/
...
void BOARD_SW_IRQ_HANDLER(void){
	GPIO_ClearPinsInterruptFlags(GPIOC, 1<<BOARD_SW3_GPIO_PIN);
	buttonpress=true;
}
int main(void)
{
    ...
    GPIO_PinInit(GPIOC, BOARD_LED_RED_GPIO_PIN, &ledconfig);
    PORT_SetPinInterruptConfig(PORTC, 4U, kPORT_InterruptFallingEdge);

    EnableIRQ(BOARD_SW3_IRQ);
    GPIO_PinInit(GPIOC, BOARD_SW3_GPIO_PIN, &swconfig);
...
static void prvQueueReceiveTask(void *pvParameters)
{
    ...
    for (;;)
    {
        ...
        if (ulReceivedValue == 100UL)
        {
            ...
            GPIO_TogglePinsOutput(GPIOC, 1<<BOARD_LED_RED_GPIO_PIN);
            buttonpress=false;
        }
    }
}
